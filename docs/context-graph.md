...
This is intentionally not a generic graph API, a second write model, agent memory, or a replacement for `able_customer_workspace`. It is a bounded retrieval layer for questions that cross modules, such as:
...
| Cloudflare capability | Role in Able | Boundary |
| --- | --- | --- |
...
| [Agents SDK state](https://developers.cloudflare.com/agents/runtime/lifecycle/state/) | Durable SQLite state per Agent instance when Able later hosts agent loops; it is session-scoped only when routing deliberately maps one session to one instance | Agent-instance memory is not shared business truth and must not replace module reads |
...
3. **One context lens.** Add `support_decision` behind a read-only interface and compare it with `able_customer_workspace` on the same evaluations.
