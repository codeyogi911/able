...
describe('voice demo boundary', () => {
  ...
  it('renders a no-store, CSP-protected page that opens without an upfront contact gate', async () => {
    ...
    expect(html).toContain('id="conversation-status"')
    expect(html).toContain('role="status" aria-live="polite" aria-atomic="true"')
    expect(html).toContain('<div id="voice-state" class="voice-state" hidden>')
    expect(html).toContain('products, orders, or how-tos.')
  ...
  it('advertises order lookup and sign-in according to configuration', async () => {
    ...
    expect(withSignIn).toContain('href="/auth/shopify/start" aria-label="Sign in for order help"')
    expect(withSignIn).toContain('data-lucide="user-round"')
  ...
  it('renders an agent-first landing with voice as a secondary control', async () => {
    ...
    expect(html).toContain('Ask Ava about Example Company products, orders, or how-tos.')
    ...
    expect(suffixedHtml).toContain('Ask Ava about Example Company products, orders, or how-tos.')
    expect(suffixedHtml).not.toContain('Ask Ava about Example Company Support products,')
    expect(html).toContain('Sales &amp; support assistant')
    ...
    expect(html).toContain('class="help-centre-link" href="/kb" aria-label="Help centre"')
    expect(html).toContain('id="human-help-button"')
    expect(html).toContain('id="conversation-human-button"')
    expect(html).toContain('class="conversation-help-button" type="button" aria-label="Contact support"')
    expect(html).not.toContain('class="human-action-button"')
    expect(html).toContain('Start with your issue; sign in only if a private request is needed.')
    expect(html).toContain('id="reconnect-banner"')
    expect(html).not.toContain('class="privacy-note"')
    expect(html).toContain('viewport-fit=cover')
    expect(html).toContain('class="welcome-visual"')
    expect(html).toContain('Find the right product. Get help after.')
    expect(html).toContain('data-lucide="package-search"')
    expect(html).toContain('<textarea id="landing-input"')
