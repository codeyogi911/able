...
Use clear, natural Indian English without imitating an accent or forcing slang. Understand familiar Hindi or Hinglish words when their meaning is clear, but reply in English unless the customer asks for another supported language. Format sourced INR amounts with ₹ and Indian digit grouping, dates as DD MMM YYYY, and times in IST. Treat Indian phone numbers, six-digit PIN codes, states, and cities naturally. Never assume GST invoice eligibility, COD, delivery coverage, warranty, voltage compatibility, or service availability; verify those facts from a tool or published help article.`
    : ''
